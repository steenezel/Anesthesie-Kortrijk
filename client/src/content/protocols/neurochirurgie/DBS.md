---
title: "Deep Brain Stimulation"
discipline: "neurochirurgie"
lastUpdated: "Augustus 2023"
---
### Indicatie:

- Parkinson (STN)
- Tremor (ViM)
- Dystonie (PGI)
- OCD, chronische pijn,…

### Preoperatieve aandachtspunten:

- Klassieke aanpassing thuismedicatie (cfr preop boekje)
- Geen preoperatieve benzodiazepines (anxiolytica)
- Aanpassing parkinson-medicatie door collega’s neurologie (zij passen EMV aan)
- Inschatten nood aan blaassonde
- Uitleg over duurtijd operatie (+- 6u lang) met het hoofd gefixeerd
- Verschillende fases van de procedure uitleggen met betrekking tot sedatie en reveil

### Peroperatief – anesthesie:

- _Fase 1 plaatsing kader Operatiezaal (19)_
  - niet-positionele IV-lijn + octopus / linkszijdige CVC (tunneling gebeurt via rechts)
  - IV bolus diprivan (1mg/kg)
  - scheren
  - LA door chirurg op 4 plaatsen op de schedel > plaatsen pinnen en positioneren kader
- _Fase 2 CT Radiologie gelijkvloers_
  - Transport naar CT met kader op schedel gefixeerd, altijd zelf begeleiden!
  - Geen sedatie!
  - schroevendraaier mee om kader los te maken bij urgentie (checken bij de vpk)
  
### Indicatie:
- Parkinson (STN)
- Tremor (ViM)
- Dystonie (PGI)
- OCD, chronische pijn,…
### Preoperatieve aandachtspunten:
- Klassieke aanpassing thuismedicatie (cfr preop boekje)
- Geen preoperatieve benzodiazepines (anxiolytica)
- Aanpassing parkinson-medicatie door collega’s neurologie (zij passen EMV aan)
- Inschatten nood aan blaassonde
- Uitleg over duurtijd operatie (+- 6u lang) met het hoofd gefixeerd
- Verschillende fases van de procedure uitleggen met betrekking tot sedatie en reveil
### Peroperatief – anesthesie:
- _Fase 1 plaatsing kader Operatiezaal (19)_
  - niet-positionele IV-lijn + octopus / linkszijdige CVC (tunneling gebeurt via rechts)
IV bolus diprivan (1mg/kg)
  - scheren
  - LA door chirurg op 4 plaatsen op de schedel > plaatsen pinnen en positioneren kader
- _Fase 2 CT Radiologie gelijkvloers_
  - Transport naar CT met kader op schedel gefixeerd, altijd zelf begeleiden!
Geen sedatie!
  - schroevendraaier mee om kader los te maken bij urgentie (checken bij de vpk)
  - Verbedden op CT: cave CWZ
- _Fase 3 installatie Operatiezaal (19)_
  - ANE: cfr infra
  - Koppelen CT – NMR, instellen richting en diepte op stereotaxie
  - Steriel afdekken
- _Fase 4 plaatsen lead (X2 indien bilateraal)_
Start IV sedatie en vermijden hypertensie
  - Incisie en boren boorgat
  - quadripolaire lead tot in target (cfr infra)
  - controle correcte positie lead mbv neuronavigatie en electrofysiologisch testing
- _Fase 5 testen lead (X2 indien bilateraal)_
Awake fase, stop sedatie zodra dura mater geopend is
  - Klinisch testing van het effect door neuroloog
  - O-arm
- _Fase 6 batterij_
  - Verwijderen kader
  - AA en ETT
  - Ankeren en tunnelen leads tot pocket buikwand + plaatsen batterij
### Locoregionale:
- Niet van toepassing
### Installatie patiënt:
- Terug in OK na CT: plaatsen arteriële, plaatsen BS zo nodig (alle vrouwen, mannen alleen als
ze niet kunnen plassen in urinaal)
- Positionering: rugligging, kader gefixeerd aan hoofdsteun, zuurstofmasker + CO2 monitoring,
2x armsteun, ondersteuning knieën, inpakken nek tegen afkoelen
- Checken IV toegang en inloopsnelheid
Inductie-onderhoudsmedicatie:
- Sedatie in fase 4:
  - Diprivan 1.0 à 3.0 TCI (PJG)
  - Midazolam 1 mg + Ultiva 0,05 mcg/kg/u + Diprivan 0,5-1,5 TCI (LC, CC)
  - Dexdor 0,4 – 1,2 mcg/kg/u
- AA in fase 6:
  - vrije keuze anesthetica, geen neuromonitoring in deze fase
Pijnbeleid:
- Klassiek postoperatief pijnbeleid
Specifieke aandachtspunten:
- Lange duur heelkunde (+/- 6u): aandacht voor temperatuur, drukpunten en comfort wakkere
patiënt
- 24u antibiotica: cefazoline 2g/8u
### Postoperatieve aandachtspunten:
- STROKE-unit (soms one-night PAZA indien geen plaats op stroke unit)
- Neurologische parameters: bewustzijn, pupillen, misselijkheid, motoriek 4 ledematen, …
- Normotensie, zo nodig met nicardipine IV
Speciale aandachtspunten medicatie:
- Geen LMWH dag 0 tenzij anders aangegeven door NCH/NEU
- Geen neuroleptica (vb droperidol)
- Anti-parkinsonmedicatie: EMV door neurologie

![Fase 2](/images/protocols/dbs_fase2.png)

![Fase 3](/images/protocols/dbs_fase3.png)

![Fase 4](/images/protocols/dbs_fase4.png)

![Fase 4](/images/protocols/dbs_fase4b.png)

![Fase 5](/images/protocols/dbs_fase5.png)

![Fase 5](/images/protocols/dbs_fase5b.png)